...
class Forum_thread_model extends Model
{
    ...
    public function setVote(int $thread_id,int $user_id,int $vote){
        ...
        $existing = $this->fetch(
            "SELECT vote FROM forum_thread_votes WHERE thread_id = :thread_id AND user_id = :user_id",['thread_id' => $thread_id,'user_id' => $user_id]
        );

        if(is_array($existing) && (int)$existing['vote'] === $vote){
            $this->query(
                "DELETE FROM forum_thread_votes WHERE thread_id = :thread_id AND user_id = :user_id",['thread_id' =>$thread_id,'user_id' => $user_id]
            );
        }
        elseif(is_array($existing)) {
            $this->query(
                "UPDATE forum_thread_votes SET vote =:vote WHERE thread_id = :thread_id AND user_id = :user_id",['vote' =>$vote,'thread_id' =>$thread_id,'user_id' =>$user_id]
            );
        }
    }
}
